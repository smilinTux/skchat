...
7. **`created_at` is tracked but not surfaced or sorted on.** `Space.created_at` is set at creation (`routes.py:185`) and exists on the model (`space.py:36`), but `GET /spaces` does not include it in its JSON (`routes.py:242-249`) and the Flutter `SpaceSummary.fromJson` has no field for it, so the directory list has no defined, newest-first ordering. It reflects dict/creation order incidentally.
...
- **`space.html`** is the room itself. Key behaviors, all verified in the markup:
  ...
  - **Version self-heal**: the HTML shell is served with `Cache-Control: no-cache, no-store, must-revalidate` (`routes.py:495`) specifically so a phone that loaded a Space before a deploy never keeps running stale JS.
