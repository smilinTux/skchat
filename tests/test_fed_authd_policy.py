...
def test_full_access_capped_to_listener_when_configured():
    out = authorize(
        _signed("opus@chef.skworld.io", "space-x"),
        sfu_ws_url="wss://h",
        _verify=_verify_to("opus@chef.skworld.io", "space-x"),
...
def test_unknown_space_is_denied():
    with pytest.raises(AuthDenied, match="space"):
        authorize(
            _signed("opus@chef.skworld.io", "space-gone"),
            sfu_ws_url="wss://h",
            _verify=_verify_to("opus@chef.skworld.io", "space-gone"),
            _access=lambda f: AccessLevel.FULL,
