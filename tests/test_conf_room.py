...
def test_named_id_is_deterministic_for_same_host_and_slug():
    a = derive_conf_id("lumina@chef.skworld.io", "standup")
    b = derive_conf_id("lumina@chef.skworld.io", "standup")
    assert a == b
...
def test_named_id_is_whitespace_insensitive():
    assert derive_conf_id(" lumina@chef.skworld.io ", " standup ") == derive_conf_id(
        "lumina@chef.skworld.io", "standup"
    )


def test_named_id_differs_by_slug_and_host():
    base = derive_conf_id("lumina@chef.skworld.io", "standup")
    assert derive_conf_id("lumina@chef.skworld.io", "retro") != base
    assert derive_conf_id("opus@chef.skworld.io", "standup") != base


def test_adhoc_id_is_random_each_call():
    ids = {derive_conf_id("lumina@chef.skworld.io") for _ in range(50)}
...
def test_conf_defaults_and_room_property():
    c = Conf(conf_id="conf-abc", host_fqid="lumina@chef.skworld.io", title="All Hands")
...
def test_create_get_list_live_end_lifecycle(tmp_path):
    ...
    conf = reg.create("lumina@chef.skworld.io", "Sprint Planning", slug="sprint")
...
def test_create_named_room_is_stable_across_registries(tmp_path):
    ...
    first = ConfRegistry(path=p).create("lumina@chef.skworld.io", "Retro", slug="retro")
    # Same (host, slug) → same id even from a fresh registry.
    again = ConfRegistry(path=p).create("lumina@chef.skworld.io", "Retro", slug="retro")
    assert first.conf_id == again.conf_id


def test_create_adhoc_rooms_differ(tmp_path):
    reg = ConfRegistry(path=tmp_path / "confs.json")
    a = reg.create("lumina@chef.skworld.io", "Quick Sync")
    b = reg.create("lumina@chef.skworld.io", "Quick Sync")
...
def test_custom_participant_cap(tmp_path):
    reg = ConfRegistry(path=tmp_path / "confs.json")
    c = reg.create("lumina@chef.skworld.io", "Town Hall", slug="th", participant_cap=100)
    assert c.participant_cap == 100


def test_persists_across_instances(tmp_path):
    p = tmp_path / "confs.json"
    c = ConfRegistry(path=p).create("lumina@chef.skworld.io", "Demo", slug="demo")
