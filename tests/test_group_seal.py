...
def _classical_group():
    ...
    group.add_member(
        identity_uri="capauth:alice@skworld.io", role=MemberRole.ADMIN, public_key_armor="PGP-KEY"
    )  # sender
    group.add_member(
        identity_uri="capauth:bob@skworld.io", public_key_armor="PGP-KEY"
    )  # keyed member
    group.add_member(
        identity_uri="capauth:carol@skworld.io", public_key_armor=""
    )  # keyless member
    return group
...
def test_apply_group_key_package_ignores_not_for_us(tmp_path, monkeypatch):
    ...
    assert (
        G.apply_group_key_package(
            {"type": "group_epoch_advance", "group_id": "g", "distributions": {}},
            self_uri="capauth:me@skworld.io",
        )
        is False
    )
