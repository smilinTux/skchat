...
def test_api_send_uses_transport_and_returns_contract(monkeypatch):
    ...
    assert transport.calls == [{"recipient": "capauth:bob@local", "content": "hello world"}]
...
def test_api_send_blank_content_is_400(monkeypatch):
    ...
    r = _client().post("/api/send", json={"recipient": "capauth:bob@local", "content": "   "})
...
def test_api_send_falls_back_to_history_when_no_transport(monkeypatch):
    ...
    r = _client().post("/api/send", json={"recipient": "capauth:bob@local", "content": "hey"})
    assert r.status_code == 200
    assert len(saved) == 1
