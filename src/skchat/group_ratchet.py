...
# vetted ratchet primitives (byte-identical to the local definitions below,
# which were lifted verbatim into sk_pqc). On boxes WITHOUT sk-pqc (e.g. .41)
# the import fails and the UNCHANGED local definitions below are used — same
# behaviour either way, so the LIVE cross-node group ratchet is unaffected.
# Hybrid security holds while EITHER leg survives; never "quantum-proof". Footer wins.
try:
    from sk_pqc.group_ratchet import *  # noqa: F401,F403  (published lib preferred)
...
if _SK_PQC_BACKED:
    from sk_pqc.group_ratchet import *  # noqa: F401,F403
